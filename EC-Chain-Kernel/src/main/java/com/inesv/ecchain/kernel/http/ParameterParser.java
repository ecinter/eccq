...
public final class ParameterParser {
    ...
    public static AccountName getAlias(HttpServletRequest req) throws ParameterException {
        long aliasId;
        try {
            aliasId = Convert.parseUnsignedLong(Convert.emptyToNull(req.getParameter("accountName")));
        } catch (RuntimeException e) {
        ...
        if (aliasId != 0) {
            accountName = AccountName.getAlias(aliasId);
    ...
    public static long getAmountNQT(HttpServletRequest req) throws ParameterException {
        return getLong(req, "amountNQT", 1L, Constants.EC_MAX_BALANCE_NQT, true);
    }

    public static long getFeeNQT(HttpServletRequest req) throws ParameterException {
        return getLong(req, "feeNQT", 0L, Constants.EC_MAX_BALANCE_NQT, true);
    }

    public static long getPriceNQT(HttpServletRequest req) throws ParameterException {
        return getLong(req, "priceNQT", 1L, Constants.EC_MAX_BALANCE_NQT, true);
    }
    ...
    public static Property getAsset(HttpServletRequest req) throws ParameterException {
        Property property = Property.getAsset(getUnsignedLong(req, "property", true));
        if (property == null) {
    ...
    public static Coin getCurrency(HttpServletRequest req) throws ParameterException {
        return getCurrency(req, true);
    }

    public static Coin getCurrency(HttpServletRequest req, boolean isMandatory) throws ParameterException {
        Coin coin = Coin.getCoin(getUnsignedLong(req, "coin", isMandatory));
        if (isMandatory && coin == null) {
            throw new ParameterException(UNKNOWN_CURRENCY);
        }
        return coin;
    }

    public static CoinBuyOffer getBuyOffer(HttpServletRequest req) throws ParameterException {
        CoinBuyOffer offer = CoinBuyOffer.getOffer(getUnsignedLong(req, "offer", true));
        if (offer == null) {
    ...
    public static CoinSellOffer getSellOffer(HttpServletRequest req) throws ParameterException {
        CoinSellOffer offer = CoinSellOffer.getOffer(getUnsignedLong(req, "offer", true));
        if (offer == null) {
            throw new ParameterException(UNKNOWN_OFFER);
        }
        return offer;
    ...
    public static long getQuantityQNT(HttpServletRequest req) throws ParameterException {
        return getLong(req, "quantityQNT", 1L, Constants.EC_MAX_ASSET_QUANTITY_QNT, true);
    }

    public static long getAmountNQTPerQNT(HttpServletRequest req) throws ParameterException {
        return getLong(req, "amountNQTPerQNT", 1L, Constants.EC_MAX_BALANCE_NQT, true);
    }

    public static ElectronicProductStore.Goods getGoods(HttpServletRequest req) throws ParameterException {
        ElectronicProductStore.Goods goods = ElectronicProductStore.Goods.getGoods(getUnsignedLong(req, "goods", true));
        if (goods == null) {
    ...
    public static int getGoodsQuantity(HttpServletRequest req) throws ParameterException {
        return getInt(req, "quantity", 0, Constants.EC_MAX_DGS_LISTING_QUANTITY, true);
    }
    ...
    public static Enclosure.EncryptToSelfMessage getEncryptToSelfMessage(HttpServletRequest req) throws ParameterException {
        ...
        if (encryptedData != null) {
            return new Enclosure.EncryptToSelfMessage(encryptedData, isText, compress);
        } else {
            return new UnencryptedEncryptToSelfMessage(plainMessageBytes, isText, compress);
        }
    }

    public static ElectronicProductStore.Purchase getPurchase(HttpServletRequest req) throws ParameterException {
        ElectronicProductStore.Purchase purchase = ElectronicProductStore.Purchase.getPurchase(getUnsignedLong(req, "purchase", true));
        if (purchase == null) {
    ...
    public static int getNumberOfConfirmations(HttpServletRequest req) throws ParameterException {
        return getInt(req, "numberOfConfirmations", 0, EcBlockchainImpl.getInstance().getHeight(), false);
    }
    ...
    public static long getHoldingId(HttpServletRequest req, HoldingType holdingType) throws ParameterException {
        long holdingId = ParameterParser.getUnsignedLong(req, "holding", holdingType != HoldingType.EC);
        if (holdingType == HoldingType.EC && holdingId != 0) {
            throw new ParameterException(incorrect("holding", "holding id should not be specified if holdingType is EC"));
        }
        return holdingId;
    }
    ...
    public static Builder parseTransaction(String transactionJSON, String transactionBytes, String prunableAttachmentJSON) throws ParameterException {
        ...
        if (transactionJSON != null) {
            try {
                ...
                return TransactionImpl.newTransactionBuilder(json);
            } catch (EcValidationException | RuntimeException | ParseException e) {
            ...
            try {
                ...
                return TransactionImpl.newTransactionBuilder(bytes, prunableAttachments);
            } catch (EcValidationException | RuntimeException | ParseException e) {
                LoggerUtil.logError(e.getMessage(), e);
                JSONObject response = new JSONObject();
    ...
    public static Enclosure getPlainMessage(HttpServletRequest req, boolean prunable) throws ParameterException {
        ...
        try {
            ...
            String detectedMimeType = Search.checkMimeType(message);
    ...
    public static Enclosure getEncryptedMessage(HttpServletRequest req, Account recipient, boolean prunable) throws ParameterException {
        ...
        if (encryptedData == null) {
            ...
            if (plainMessage == null) {
                ...
                try {
                    ...
                    String detectedMimeType = Search.checkMimeType(plainMessageBytes);
                    if (detectedMimeType != null) {
        ...
        if (encryptedData != null) {
            if (prunable) {
                return new Enclosure.PrunableEncryptedMessage(encryptedData, isText, compress);
            } else {
                return new Enclosure.EncryptedMessage(encryptedData, isText, compress);
            }
        } else {
            if (prunable) {
                return new Enclosure.UnencryptedPrunableEncryptedMessage(plainMessageBytes, isText, compress, recipientPublicKey);
            } else {
    ...
    public static Mortgaged.TaggedDataUpload getTaggedData(HttpServletRequest req) throws ParameterException, EcNotValidExceptionEc {
        ...
        String detectedMimeType = Search.checkMimeType(data, filename);
        if (detectedMimeType != null) {
            isText = detectedMimeType.startsWith("text/");
            if (type.isEmpty()) {
                type = detectedMimeType.substring(0, Math.min(detectedMimeType.length(), Constants.EC_MAX_TAGGED_DATA_TYPE_LENGTH));
            }
        }
        ...
        name = name.trim();
        if (name.length() > Constants.EC_MAX_TAGGED_DATA_NAME_LENGTH) {
            throw new ParameterException(INCORRECT_TAGGED_DATA_NAME);
        }

        if (description.length() > Constants.EC_MAX_TAGGED_DATA_DESCRIPTION_LENGTH) {
            throw new ParameterException(INCORRECT_TAGGED_DATA_DESCRIPTION);
        }

        if (tags.length() > Constants.EC_MAX_TAGGED_DATA_TAGS_LENGTH) {
            throw new ParameterException(INCORRECT_TAGGED_DATA_TAGS);
        }

        type = type.trim();
        if (type.length() > Constants.EC_MAX_TAGGED_DATA_TYPE_LENGTH) {
            throw new ParameterException(INCORRECT_TAGGED_DATA_TYPE);
        }

        channel = channel.trim();
        if (channel.length() > Constants.EC_MAX_TAGGED_DATA_CHANNEL_LENGTH) {
            throw new ParameterException(INCORRECT_TAGGED_DATA_CHANNEL);
        }

        if (data.length == 0 || data.length > Constants.EC_MAX_TAGGED_DATA_DATA_LENGTH) {
            throw new ParameterException(INCORRECT_DATA);
        }

        if (filename.length() > Constants.EC_MAX_TAGGED_DATA_FILENAME_LENGTH) {
            throw new ParameterException(INCORRECT_TAGGED_DATA_FILENAME);
        }
        return new Mortgaged.TaggedDataUpload(name, description, tags, type, channel, isText, filename, data);
    }
