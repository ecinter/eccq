...
public interface Mortgaged extends Enclosure {

    EmptyMortgaged ORDINARY_PAYMENT = new EmptyMortgaged() {
    ...
    // the message payload is in the Enclosure
    EmptyMortgaged ARBITRARY_MESSAGE = new EmptyMortgaged() {
    ...
    interface ShufflingMortgaged extends Mortgaged {
    ...
    abstract class AbstractMortgaged extends AbstractEnclosure implements Mortgaged {

        private AbstractMortgaged(ByteBuffer buffer, byte transactionVersion) {
            super(buffer, transactionVersion);
        }

        private AbstractMortgaged(JSONObject attachmentData) {
            super(attachmentData);
        }

        private AbstractMortgaged(int version) {
            super(version);
        }

        private AbstractMortgaged() {
        }
        ...
        @Override
        final void validate(Transaction transaction) throws EcValidationException {
        ...
        final int getFinishValidationHeight(Transaction transaction) {
            return isPhased(transaction) ? transaction.getPhasing().getFinishHeight() - 1 : EcBlockchainImpl.getInstance().getHeight();
        }

    }

    abstract class EmptyMortgaged extends AbstractMortgaged {

        private EmptyMortgaged() {
            super(0);
        }
        ...
        final boolean verifyVersion(byte transactionVersion) {
            return getEcVersion() == 0;
        }

    }

    final class MessagingAliasAssignment extends AbstractMortgaged {

        private final String aliasName;
        private final String aliasURI;

        MessagingAliasAssignment(ByteBuffer buffer, byte transactionVersion) throws EcNotValidExceptionEc {
            super(buffer, transactionVersion);
            aliasName = Convert.readString(buffer, buffer.get(), Constants.EC_MAX_ALIAS_LENGTH).trim();
            aliasURI = Convert.readString(buffer, buffer.getShort(), Constants.EC_MAX_ALIAS_URI_LENGTH).trim();
        }
    ...
    final class MessagingAliasSell extends AbstractMortgaged {

        private final String aliasName;
        private final long priceNQT;

        MessagingAliasSell(ByteBuffer buffer, byte transactionVersion) throws EcNotValidExceptionEc {
            super(buffer, transactionVersion);
            this.aliasName = Convert.readString(buffer, buffer.get(), Constants.EC_MAX_ALIAS_LENGTH);
    ...
    final class MessagingAliasBuy extends AbstractMortgaged {

        private final String aliasName;

        MessagingAliasBuy(ByteBuffer buffer, byte transactionVersion) throws EcNotValidExceptionEc {
            super(buffer, transactionVersion);
            this.aliasName = Convert.readString(buffer, buffer.get(), Constants.EC_MAX_ALIAS_LENGTH);
        }
    ...
    final class MessagingAliasDelete extends AbstractMortgaged {

        private final String aliasName;

        MessagingAliasDelete(final ByteBuffer buffer, final byte transactionVersion) throws EcNotValidExceptionEc {
            super(buffer, transactionVersion);
            this.aliasName = Convert.readString(buffer, buffer.get(), Constants.EC_MAX_ALIAS_LENGTH);
        }
    ...
    final class MessagingPollCreation extends AbstractMortgaged {
        ...
        MessagingPollCreation(ByteBuffer buffer, byte transactionVersion) throws EcNotValidExceptionEc {
            super(buffer, transactionVersion);
            this.pollName = Convert.readString(buffer, buffer.getShort(), Constants.EC_MAX_POLL_NAME_LENGTH);
            this.pollDescription = Convert.readString(buffer, buffer.getShort(), Constants.EC_MAX_POLL_DESCRIPTION_LENGTH);

            this.finishHeight = buffer.getInt();

            int numberOfOptions = buffer.get();
            if (numberOfOptions > Constants.EC_MAX_POLL_OPTION_COUNT) {
                throw new EcNotValidExceptionEc("Invalid number of poll options: " + numberOfOptions);
            }

            this.pollOptions = new String[numberOfOptions];
            for (int i = 0; i < numberOfOptions; i++) {
                this.pollOptions[i] = Convert.readString(buffer, buffer.getShort(), Constants.EC_MAX_POLL_OPTION_LENGTH);
            }
    ...
    final class MessagingVoteCasting extends AbstractMortgaged {

        private final long pollId;
        private final byte[] pollVote;

        public MessagingVoteCasting(ByteBuffer buffer, byte transactionVersion) throws EcNotValidExceptionEc {
            super(buffer, transactionVersion);
            pollId = buffer.getLong();
            int numberOfOptions = buffer.get();
            if (numberOfOptions > Constants.EC_MAX_POLL_OPTION_COUNT) {
                throw new EcNotValidExceptionEc("More than " + Constants.EC_MAX_POLL_OPTION_COUNT + " options in a vote");
            }
    ...
    final class MessagingPhasingVoteCasting extends AbstractMortgaged {

        private final List<byte[]> transactionFullHashes;
        private final byte[] revealedSecret;

        MessagingPhasingVoteCasting(ByteBuffer buffer, byte transactionVersion) throws EcNotValidExceptionEc {
            super(buffer, transactionVersion);
            ...
            if (secretLength > Constants.EC_MAX_PHASING_REVEALED_SECRET_LENGTH) {
                throw new EcNotValidExceptionEc("Invalid revealed secret length " + secretLength);
            }
            if (secretLength > 0) {
                revealedSecret = new byte[secretLength];
                buffer.get(revealedSecret);
            } else {
                revealedSecret = Convert.EC_EMPTY_BYTE;
            }
        }
        ...
        MessagingPhasingVoteCasting(JSONObject attachmentData) {
            ...
            this.revealedSecret = revealedSecret != null ? Convert.parseHexString(revealedSecret) : Convert.EC_EMPTY_BYTE;
        }
    ...
    final class MessagingHubAnnouncement extends AbstractMortgaged {

        private final long minFeePerByteNQT;
        private final String[] uris;

        MessagingHubAnnouncement(ByteBuffer buffer, byte transactionVersion) throws EcNotValidExceptionEc {
            super(buffer, transactionVersion);
            this.minFeePerByteNQT = buffer.getLong();
            int numberOfUris = buffer.get();
            if (numberOfUris > Constants.EC_MAX_HUB_ANNOUNCEMENT_URIS) {
                throw new EcNotValidExceptionEc("Invalid number of URIs: " + numberOfUris);
            }
            this.uris = new String[numberOfUris];
            for (int i = 0; i < uris.length; i++) {
                uris[i] = Convert.readString(buffer, buffer.getShort(), Constants.EC_MAX_HUB_ANNOUNCEMENT_URI_LENGTH);
            }
        }

        MessagingHubAnnouncement(JSONObject attachmentData) throws EcNotValidExceptionEc {
            super(attachmentData);
            ...
            try {
                ...
                throw new EcNotValidExceptionEc("Error parsing heart terminal announcement parameters", e);
            }
        }
    ...
    final class MessagingAccountInfo extends AbstractMortgaged {

        private final String name;
        private final String description;

        MessagingAccountInfo(ByteBuffer buffer, byte transactionVersion) throws EcNotValidExceptionEc {
            super(buffer, transactionVersion);
            this.name = Convert.readString(buffer, buffer.get(), Constants.EC_MAX_ACCOUNT_NAME_LENGTH);
            this.description = Convert.readString(buffer, buffer.getShort(), Constants.EC_MAX_ACCOUNT_DESCRIPTION_LENGTH);
        }
    ...
    final class MessagingAccountProperty extends AbstractMortgaged {

        private final String property;
        private final String value;

        MessagingAccountProperty(ByteBuffer buffer, byte transactionVersion) throws EcNotValidExceptionEc {
            super(buffer, transactionVersion);
            this.property = Convert.readString(buffer, buffer.get(), Constants.EC_MAX_ACCOUNT_PROPERTY_NAME_LENGTH).trim();
            this.value = Convert.readString(buffer, buffer.get(), Constants.EC_MAX_ACCOUNT_PROPERTY_VALUE_LENGTH).trim();
        }
    ...
    final class MessagingAccountPropertyDelete extends AbstractMortgaged {
    ...
    final class ColoredCoinsAssetIssuance extends AbstractMortgaged {
        ...
        ColoredCoinsAssetIssuance(ByteBuffer buffer, byte transactionVersion) throws EcNotValidExceptionEc {
            super(buffer, transactionVersion);
            this.name = Convert.readString(buffer, buffer.get(), Constants.EC_MAX_ASSET_NAME_LENGTH);
            this.description = Convert.readString(buffer, buffer.getShort(), Constants.EC_MAX_ASSET_DESCRIPTION_LENGTH);
    ...
    final class ColoredCoinsAssetTransfer extends AbstractMortgaged {

        private final long assetId;
        private final long quantityQNT;
        private final String comment;

        ColoredCoinsAssetTransfer(ByteBuffer buffer, byte transactionVersion) throws EcNotValidExceptionEc {
            super(buffer, transactionVersion);
            this.assetId = buffer.getLong();
            this.quantityQNT = buffer.getLong();
            this.comment = getEcVersion() == 0 ? Convert.readString(buffer, buffer.getShort(), Constants.EC_MAX_ASSET_TRANSFER_COMMENT_LENGTH) : null;
        }
        ...
        ColoredCoinsAssetTransfer(JSONObject attachmentData) {
            ...
            this.quantityQNT = Convert.parseLong(attachmentData.get("quantityQNT"));
            this.comment = getEcVersion() == 0 ? Convert.nullToEmpty((String) attachmentData.get("comment")) : null;
        }
        ...
        @Override
        int getMySize() {
            return 8 + 8 + (getEcVersion() == 0 ? (2 + Convert.toBytes(comment).length) : 0);
        }
        ...
        void putMyBytes(ByteBuffer buffer) {
            ...
            buffer.putLong(quantityQNT);
            if (getEcVersion() == 0 && comment != null) {
    ...
    final class ColoredCoinsAssetDelete extends AbstractMortgaged {
    ...
    abstract class ColoredCoinsOrderPlacement extends AbstractMortgaged {
    ...
    abstract class ColoredCoinsOrderCancellation extends AbstractMortgaged {
    ...
    final class ColoredCoinsDividendPayment extends AbstractMortgaged {
    ...
    final class DigitalGoodsListing extends AbstractMortgaged {
        ...
        DigitalGoodsListing(ByteBuffer buffer, byte transactionVersion) throws EcNotValidExceptionEc {
            super(buffer, transactionVersion);
            this.name = Convert.readString(buffer, buffer.getShort(), Constants.EC_MAX_DGS_LISTING_NAME_LENGTH);
            this.description = Convert.readString(buffer, buffer.getShort(), Constants.EC_MAX_DGS_LISTING_DESCRIPTION_LENGTH);
            this.tags = Convert.readString(buffer, buffer.getShort(), Constants.EC_MAX_DGS_LISTING_TAGS_LENGTH);
    ...
    final class DigitalGoodsDelisting extends AbstractMortgaged {
    ...
    final class DigitalGoodsPriceChange extends AbstractMortgaged {
    ...
    final class DigitalGoodsQuantityChange extends AbstractMortgaged {
    ...
    final class DigitalGoodsPurchase extends AbstractMortgaged {
    ...
    class DigitalGoodsDelivery extends AbstractMortgaged {
        ...
        DigitalGoodsDelivery(ByteBuffer buffer, byte transactionVersion) throws EcNotValidExceptionEc {
            super(buffer, transactionVersion);
            ...
            this.goods = EncryptedData.readEcEncryptedData(buffer, length, Constants.EC_MAX_DGS_GOODS_LENGTH);
    ...
    final class UnencryptedDigitalGoodsDelivery extends DigitalGoodsDelivery implements Encryptable {
        ...
        void putMyBytes(ByteBuffer buffer) {
            if (getGoods() == null) {
                throw new EcNotYetEncryptedException("Goods not yet encrypted");
            }
    ...
    final class DigitalGoodsFeedback extends AbstractMortgaged {
    ...
    final class DigitalGoodsRefund extends AbstractMortgaged {
    ...
    final class AccountControlEffectiveBalanceLeasing extends AbstractMortgaged {
    ...
    final class MonetarySystemCurrencyIssuance extends AbstractMortgaged {
        ...
        MonetarySystemCurrencyIssuance(ByteBuffer buffer, byte transactionVersion) throws EcNotValidExceptionEc {
            super(buffer, transactionVersion);
            this.name = Convert.readString(buffer, buffer.get(), Constants.EC_MAX_CURRENCY_NAME_LENGTH);
            this.code = Convert.readString(buffer, buffer.get(), Constants.EC_MAX_CURRENCY_CODE_LENGTH);
            this.description = Convert.readString(buffer, buffer.getShort(), Constants.EC_MAX_CURRENCY_DESCRIPTION_LENGTH);
    ...
    final class MonetarySystemReserveIncrease extends AbstractMortgaged implements MonetarySystemAttachment {
    ...
    final class MonetarySystemReserveClaim extends AbstractMortgaged implements MonetarySystemAttachment {
    ...
    final class MonetarySystemCurrencyTransfer extends AbstractMortgaged implements MonetarySystemAttachment {
    ...
    final class MonetarySystemPublishExchangeOffer extends AbstractMortgaged implements MonetarySystemAttachment {
    ...
    abstract class MonetarySystemExchange extends AbstractMortgaged implements MonetarySystemAttachment {
    ...
    final class MonetarySystemCurrencyMinting extends AbstractMortgaged implements MonetarySystemAttachment {
    ...
    final class MonetarySystemCurrencyDeletion extends AbstractMortgaged implements MonetarySystemAttachment {
    ...
    final class ShufflingCreation extends AbstractMortgaged {
    ...
    abstract class AbstractShufflingMortgaged extends AbstractMortgaged implements ShufflingMortgaged {

        private final long shufflingId;
        private final byte[] shufflingStateHash;

        private AbstractShufflingMortgaged(ByteBuffer buffer, byte transactionVersion) {
            super(buffer, transactionVersion);
        ...
        private AbstractShufflingMortgaged(JSONObject attachmentData) {
            super(attachmentData);
        ...
        private AbstractShufflingMortgaged(long shufflingId, byte[] shufflingStateHash) {
    ...
    final class ShufflingRegistration extends AbstractMortgaged implements ShufflingMortgaged {
    ...
    final class ShufflingProcessing extends AbstractShufflingMortgaged implements Prunable {
        ...
        ShufflingProcessing(ByteBuffer buffer, byte transactionVersion) {
            ...
            buffer.get(hash);
            this.data = Arrays.equals(hash, emptyDataHash) ? Convert.EC_EMPTY_BYTES : null;
        }
        ...
        static ShufflingProcessing parse(JSONObject attachmentData) {
            if (!Enclosure.hasEnclosure(ShufflingTransaction.SHUFFLING_PROCESSING.getName(), attachmentData)) {
                return null;
            }
    ...
    final class ShufflingRecipients extends AbstractShufflingMortgaged {

        private final byte[][] recipientPublicKeys;

        ShufflingRecipients(ByteBuffer buffer, byte transactionVersion) throws EcNotValidExceptionEc {
            super(buffer, transactionVersion);
            int count = buffer.get();
            if (count > Constants.EC_MAX_NUMBER_OF_SHUFFLING_PARTICIPANTS || count < 0) {
                throw new EcNotValidExceptionEc("Invalid data count " + count);
            }
    ...
    final class ShufflingVerification extends AbstractShufflingMortgaged {
    ...
    final class ShufflingCancellation extends AbstractShufflingMortgaged {

        private final byte[][] blameData;
        private final byte[][] keySeeds;
        private final long cancellingAccountId;

        ShufflingCancellation(ByteBuffer buffer, byte transactionVersion) throws EcNotValidExceptionEc {
            super(buffer, transactionVersion);
            int count = buffer.get();
            if (count > Constants.EC_MAX_NUMBER_OF_SHUFFLING_PARTICIPANTS || count <= 0) {
                throw new EcNotValidExceptionEc("Invalid data count " + count);
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (size > Constants.EC_MAX_PAYLOAD_LENGTH) {
                    throw new EcNotValidExceptionEc("Invalid data size " + size);
                }
            ...
            count = buffer.get();
            if (count > Constants.EC_MAX_NUMBER_OF_SHUFFLING_PARTICIPANTS || count <= 0) {
                throw new EcNotValidExceptionEc("Invalid keySeeds count " + count);
            }
    ...
    abstract class TaggedDataMortgaged extends AbstractMortgaged implements Prunable {
        ...
        private volatile BadgeData badgeData;

        private TaggedDataMortgaged(ByteBuffer buffer, byte transactionVersion) {
            super(buffer, transactionVersion);
            this.name = null;
        ...
        private TaggedDataMortgaged(JSONObject attachmentData) {
            super(attachmentData);
        ...
        private TaggedDataMortgaged(String name, String description, String tags, String type, String channel, boolean isText, String filename, byte[] data) {
            this.name = name;
            this.description = description;
            this.tags = tags;
            this.type = type;
        ...
        void putMyJSON(JSONObject attachment) {
            if (badgeData != null) {
                attachment.put("name", badgeData.getName());
                attachment.put("description", badgeData.getDescription());
                attachment.put("tags", badgeData.getTags());
                attachment.put("type", badgeData.getType());
                attachment.put("channel", badgeData.getChannel());
                attachment.put("isText", badgeData.isText());
                attachment.put("filename", badgeData.getFilename());
                attachment.put("data", badgeData.isText() ? Convert.toString(badgeData.getData()) : Convert.toHexString(badgeData.getData()));
            } else if (data != null) {
                attachment.put("name", name);
                attachment.put("description", description);
                attachment.put("tags", tags);
        ...
        public final String getDescription() {
            if (badgeData != null) {
                return badgeData.getDescription();
            }
            return description;
        }
        ...
        @Override
        void loadPrunable(Transaction transaction, boolean includeExpiredPrunable) {
            if (data == null && badgeData == null && shouldLoadPrunable(transaction, includeExpiredPrunable)) {
                badgeData = BadgeData.getData(getTaggedDataId(transaction));
            }
        }

        @Override
        public boolean hasPrunableData() {
            return (badgeData != null || data != null);
        }
    ...
    final class TaggedDataUpload extends TaggedDataMortgaged {
        ...
        public TaggedDataUpload(String name, String description, String tags, String type, String channel, boolean isText,
                                String filename, byte[] data) throws EcNotValidExceptionEc {
            ...
            if (isText && !Arrays.equals(data, Convert.toBytes(Convert.toString(data)))) {
                throw new EcNotValidExceptionEc("Data is not UTF-8 text");
            }
        }

        static TaggedDataUpload parse(JSONObject attachmentData) {
            if (!Enclosure.hasEnclosure(Data.TAGGED_DATA_UPLOAD.getName(), attachmentData)) {
                return null;
            }
        ...
        @Override
        public void restorePrunableData(Transaction transaction, int blockTimestamp, int height) {
            BadgeData.restore(transaction, this, blockTimestamp, height);
        }

    }

    final class TaggedDataExtend extends TaggedDataMortgaged {
        ...
        TaggedDataExtend(JSONObject attachmentData) {
            super(attachmentData);
            this.taggedDataId = Convert.parseUnsignedLong((String) attachmentData.get("badgeData"));
            this.jsonIsPruned = attachmentData.get("data") == null;
        }

        public TaggedDataExtend(BadgeData badgeData) {
            super(badgeData.getName(), badgeData.getDescription(), badgeData.getTags(), badgeData.getType(),
                    badgeData.getChannel(), badgeData.isText(), badgeData.getFilename(), badgeData.getData());
            this.taggedDataId = badgeData.getId();
            this.jsonIsPruned = false;
        }

        static TaggedDataExtend parse(JSONObject attachmentData) {
            if (!Enclosure.hasEnclosure(Data.TAGGED_DATA_EXTEND.getName(), attachmentData)) {
                return null;
            }
        ...
        void putMyJSON(JSONObject attachment) {
            super.putMyJSON(attachment);
            attachment.put("badgeData", Long.toUnsignedString(taggedDataId));
        }
        ...
        public byte[] getHash() {
            ...
            if (hash == null) {
                TaggedDataUpload taggedDataUpload = (TaggedDataUpload) TransactionH2.selectTransaction(taggedDataId).getAttachment();
    ...
    final class SetPhasingOnly extends AbstractMortgaged {
