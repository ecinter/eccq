...
final class TransactionH2 {

    static TransactionImpl selectTransaction(long transactionId) {
        return selectTransaction(transactionId, Integer.MAX_VALUE);
    }

    static TransactionImpl selectTransaction(long transactionId, int height) {
        // Check the block cache
        synchronized (EcBlockH2.blockCache) {
            TransactionImpl transaction = EcBlockH2.transactionCache.get(transactionId);
            if (transaction != null) {
                return transaction.getTransactionHeight() <= height ? transaction : null;
            }
        }
        // Search the database
    ...
    static TransactionImpl selectTransactionByFullHash(byte[] fullHash) {
        return selectTransactionByFullHash(fullHash, Integer.MAX_VALUE);
    }

    static TransactionImpl selectTransactionByFullHash(byte[] fullHash, int height) {
        long transactionId = Convert.fullhashtoid(fullHash);
        // Check the cache
        synchronized (EcBlockH2.blockCache) {
            TransactionImpl transaction = EcBlockH2.transactionCache.get(transactionId);
            if (transaction != null) {
                return (transaction.getTransactionHeight() <= height &&
    ...
    static TransactionImpl loadTransaction(Connection con, ResultSet rs) throws EcNotValidExceptionEc {
        try {
    ...
    static List<TransactionImpl> selectBlockTransactions(long blockId) {
        // Check the block cache
        synchronized (EcBlockH2.blockCache) {
        ...
        // Search the database
        try (Connection con = H2.H2.getConnection()) {
            return selectBlockTransactions(con, blockId);
        } catch (SQLException e) {
            throw new RuntimeException(e.toString(), e);
        }
    }

    static List<TransactionImpl> selectBlockTransactions(Connection con, long blockId) {
    ...
    static List<PrunableTransaction> selectPrunableTransactions(Connection con, int minTimestamp, int maxTimestamp) {
        ...
        try {
            ...
            for (TransactionImpl transaction : transactions) {
                try (PreparedStatement pstmt = con.prepareStatement("INSERT INTO transaction (id, deadline, "
                    ...
                    H2Utils.h2setLongZeroToNull(pstmt, ++i, transaction.getRecipientId());
                    pstmt.setLong(++i, transaction.getAmountNQT());
                    pstmt.setLong(++i, transaction.getFeeNQT());
                    H2Utils.h2setBytes(pstmt, ++i, transaction.referencedTransactionFullHash());
                    ...
                    pstmt.setByte(++i, transaction.getTransactionType().getType());
                    pstmt.setByte(++i, transaction.getTransactionType().getSubtype());
                    pstmt.setLong(++i, transaction.getSenderId());
                    int bytesLength = 0;
                    for (Enclosure appendage : transaction.getAppendages()) {
                    ...
                    H2Utils.h2setLongZeroToNull(pstmt, ++i, transaction.getECBlockId());
