...
public final class PrunableMessage {
    ...
    private static final H2KeyLongKeyFactory<PrunableMessage> PRUNABLE_MESSAGE_KEY_FACTORY = new H2KeyLongKeyFactory<PrunableMessage>("Id") {

        @Override
        public H2Key newKey(PrunableMessage prunableMessage) {
            return prunableMessage.h2Key;
        }

    };

    private static final PrunableH2Table<PrunableMessage> PRUNABLE_MESSAGE_TABLE = new PrunableH2Table<PrunableMessage>("prunable_message", PRUNABLE_MESSAGE_KEY_FACTORY) {

        @Override
        protected PrunableMessage load(Connection con, ResultSet rs, H2Key h2Key) throws SQLException {
            return new PrunableMessage(rs, h2Key);
        }
    ...
    private PrunableMessage(Transaction transaction, int blockTimestamp, int height) {
        ...
        this.h2Key = PRUNABLE_MESSAGE_KEY_FACTORY.newKey(this.id);
    ...
    private PrunableMessage(ResultSet rs, H2Key h2Key) throws SQLException {
    ...
    public static int getCount() {
        return PRUNABLE_MESSAGE_TABLE.getCount();
    }

    public static H2Iterator<PrunableMessage> getAll(int from, int to) {
        return PRUNABLE_MESSAGE_TABLE.getAll(from, to);
    }

    public static PrunableMessage getPrunableMessage(long transactionId) {
        return PRUNABLE_MESSAGE_TABLE.get(PRUNABLE_MESSAGE_KEY_FACTORY.newKey(transactionId));
    }

    public static H2Iterator<PrunableMessage> getPrunableMessages(long accountId, int from, int to) {
        Connection con = null;
        try {
            ...
            return PRUNABLE_MESSAGE_TABLE.getManyBy(con, pstmt, false);
    ...
    public static H2Iterator<PrunableMessage> getPrunableMessages(long accountId, long otherAccountId, int from, int to) {
        Connection con = null;
        try {
            con = H2.H2.getConnection();
    ...
    static void add(TransactionImpl transaction, PrunablePlainMessage appendix) {
        add(transaction, appendix, EcBlockchainImpl.getInstance().getLastBlockTimestamp(), EcBlockchainImpl.getInstance().getHeight());
    }

    static void add(TransactionImpl transaction, PrunablePlainMessage appendix, int blockTimestamp, int height) {
        if (appendix.getMessage() != null) {
            PrunableMessage prunableMessage = PRUNABLE_MESSAGE_TABLE.get(transaction.getH2Key());
            ...
            if (prunableMessage.getMessage() == null) {
                ...
                PRUNABLE_MESSAGE_TABLE.insert(prunableMessage);
            }
        }
    }

    static void add(TransactionImpl transaction, Enclosure.PrunableEncryptedMessage appendix) {
        add(transaction, appendix, EcBlockchainImpl.getInstance().getLastBlockTimestamp(), EcBlockchainImpl.getInstance().getHeight());
    }

    static void add(TransactionImpl transaction, Enclosure.PrunableEncryptedMessage appendix, int blockTimestamp, int height) {
    ...
    private void setEncrypted(Enclosure.PrunableEncryptedMessage appendix) {
    ...
    private void save(Connection con) throws SQLException {
        ...
        try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO prunable_message (id, sender_id, recipient_id, "
            ...
            H2Utils.h2setLongZeroToNull(pstmt, ++i, this.recipientId);
            H2Utils.h2setBytes(pstmt, ++i, this.message);
            H2Utils.h2setBytes(pstmt, ++i, this.encryptedData == null ? null : this.encryptedData.getBytes());
