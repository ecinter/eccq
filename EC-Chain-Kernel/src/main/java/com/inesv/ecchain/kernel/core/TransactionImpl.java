...
public final class TransactionImpl implements Transaction {
    ...
    private final Mortgaged.AbstractMortgaged attachment;
    private final Message message;
    private final Enclosure.EncryptedMessage encryptedMessage;
    private final Enclosure.EncryptToSelfMessage encryptToSelfMessage;
    private final PublicKeyAnnouncement publicKeyAnnouncement;
    private final Phasing phasing;
    private final PrunablePlainMessage prunablePlainMessage;
    private final Enclosure.PrunableEncryptedMessage prunableEncryptedMessage;
    private final List<AbstractEnclosure> appendages;
    private final int appendagesSize;
    private volatile byte[] senderPublicKey;
    private volatile int height = Integer.MAX_VALUE;
    private volatile long blockId;
    private volatile EcBlockImpl block;
    ...
    private volatile H2Key h2Key;
    private volatile byte[] bytes = null;
    private volatile boolean hasValidSignature = false;


    private TransactionImpl(BuilderImpl builder, String secretPhrase) throws EcNotValidExceptionEc {
        ...
        List<AbstractEnclosure> list = new ArrayList<>();
        ...
        for (Enclosure appendage : appendages) {
            if (secretPhrase != null && appendage instanceof Encryptable) {
                ((Encryptable) appendage).encrypt(secretPhrase);
            }
            appendagesSize += appendage.getSize();
        }
        this.appendagesSize = appendagesSize;
        if (builder.feeNQT <= 0 || (Constants.EC_CORRECT_INVALID_FEES && builder.signature == null)) {
            int effectiveHeight = (height < Integer.MAX_VALUE ? height : EcBlockchainImpl.getInstance().getHeight());
        ...
        if (builder.signature != null && secretPhrase != null) {
            throw new EcNotValidExceptionEc("Transaction is already signed");
            ...
            if (getSenderPublicKey() != null && !Arrays.equals(senderPublicKey, Crypto.getPublicKey(secretPhrase))) {
                throw new EcNotValidExceptionEc("Secret phrase doesn't match transaction sender public key");
            }
            signature = Crypto.ecSign(bytes(), secretPhrase);
            bytes = null;
        } else {
            signature = null;
        }
    ...
    public static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes) throws EcNotValidExceptionEc {
        try {
            ...
            long amountNQT = buffer.getLong();//TODO
            ...
            TransactionType transactionType = TransactionType.selectTransactionType(type, subtype);
            ...
            if ((flags & position) != 0 || (version == 0 && transactionType == Messaging.ARBITRARY_MESSAGE)) {
                builder.appendix(new Message(buffer, version));
            }
            position <<= 1;
            if ((flags & position) != 0) {
                builder.appendix(new Enclosure.EncryptedMessage(buffer, version));
            }
            position <<= 1;
            if ((flags & position) != 0) {
                builder.appendix(new PublicKeyAnnouncement(buffer, version));
            }
            position <<= 1;
            if ((flags & position) != 0) {
                builder.appendix(new Enclosure.EncryptToSelfMessage(buffer, version));
            }
            position <<= 1;
            if ((flags & position) != 0) {
                builder.appendix(new Phasing(buffer, version));
            }
            position <<= 1;
            if ((flags & position) != 0) {
                builder.appendix(new PrunablePlainMessage(buffer, version));
            }
            position <<= 1;
            if ((flags & position) != 0) {
                builder.appendix(new Enclosure.PrunableEncryptedMessage(buffer, version));
            }
            if (buffer.hasRemaining()) {
                throw new EcNotValidExceptionEc("Transaction bytes too long, " + buffer.remaining() + " extra bytes");
            }
            return builder;
        } catch (EcNotValidExceptionEc | RuntimeException e) {
            LoggerUtil.logDebug("Failed to parse transaction bytes: " + Convert.toHexString(bytes));
            throw e;
        }
    }

    public static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes, JSONObject prunableAttachments) throws EcNotValidExceptionEc {
        BuilderImpl builder = newTransactionBuilder(bytes);
        if (prunableAttachments != null) {
            Mortgaged.ShufflingProcessing shufflingProcessing = Mortgaged.ShufflingProcessing.parse(prunableAttachments);
            if (shufflingProcessing != null) {
                builder.appendix(shufflingProcessing);
            }
            Mortgaged.TaggedDataUpload taggedDataUpload = Mortgaged.TaggedDataUpload.parse(prunableAttachments);
            if (taggedDataUpload != null) {
                builder.appendix(taggedDataUpload);
            }
            Mortgaged.TaggedDataExtend taggedDataExtend = Mortgaged.TaggedDataExtend.parse(prunableAttachments);
            ...
            Enclosure.PrunableEncryptedMessage prunableEncryptedMessage = Enclosure.PrunableEncryptedMessage.parse(prunableAttachments);
    ...
    public static TransactionImpl parseTransaction(JSONObject transactionData) throws EcNotValidExceptionEc {
        ...
        if (transaction.getSignature() != null && !transaction.checkSignature()) {
            throw new EcNotValidExceptionEc("Invalid transaction signature for transaction " + transaction.getJSONObject().toJSONString());
        }
        return transaction;
    }

    public static TransactionImpl.BuilderImpl newTransactionBuilder(JSONObject transactionData) throws EcNotValidExceptionEc {
        try {
            ...
            if (transactionType == null) {
                throw new EcNotValidExceptionEc("Invalid transaction type: " + type + ", " + subtype);
            }
            ...
            if (attachmentData != null) {
                ...
                builder.appendix(Enclosure.EncryptedMessage.parse(attachmentData));
                builder.appendix((PublicKeyAnnouncement.parse(attachmentData)));
                builder.appendix(Enclosure.EncryptToSelfMessage.parse(attachmentData));
                builder.appendix(Phasing.parse(attachmentData));
                builder.appendix(PrunablePlainMessage.parse(attachmentData));
                builder.appendix(Enclosure.PrunableEncryptedMessage.parse(attachmentData));
            }
            return builder;
        } catch (EcNotValidExceptionEc | RuntimeException e) {
            LoggerUtil.logDebug("Failed to parse transaction: " + transactionData.toJSONString());
            throw e;
        }
    }
    ...
    public EcBlockImpl getBlock() {
        if (block == null && blockId != 0) {
            block = EcBlockchainImpl.getInstance().getBlock(blockId);
        }
        return block;
    }

    void setBlock(EcBlockImpl block) {
        this.block = block;
        this.blockId = block.getECId();
    ...
    @Override
    public Mortgaged.AbstractMortgaged getAttachment() {
        attachment.loadPrunable(this);
        return attachment;
    }

    @Override
    public List<AbstractEnclosure> getAppendages() {
        return getAppendages(false);
    }

    @Override
    public List<AbstractEnclosure> getAppendages(boolean includeExpiredPrunable) {
        for (AbstractEnclosure appendage : appendages) {
    ...
    @Override
    public List<Enclosure> getAppendages(Filter<Enclosure> filter, boolean includeExpiredPrunable) {
    ...
    H2Key getH2Key() {
        if (h2Key == null) {
            h2Key = TransactionProcessorImpl.getInstance().unconfirmedtransactiondbkeyfactory.newKey(getTransactionId());
        }
        return h2Key;
    }
    ...
    byte[] bytes() {
        if (bytes == null) {
            try {
                ...
                buffer.putLong(type.canHaveRecipient() ? recipientId : Genesis.EC_CREATOR_ID);
                if (useNQT()) {
                    ...
                    buffer.putInt((int) (amountNQT / Constants.ONE_EC));
                    buffer.putInt((int) (feeNQT / Constants.ONE_EC));
                    if (referencedTransactionFullHash != null) {
                ...
                if (signature != null) {
                    LoggerUtil.logDebug("Failed to get transaction bytes for transaction: " + getJSONObject().toJSONString());
                }
                throw e;
            }
        }
    ...
    public boolean equals(Object o) {
        return o instanceof TransactionImpl && this.getTransactionId() == ((Transaction) o).getTransactionId();
    }

    @Override
    public int hashCode() {
        return (int) (getTransactionId() ^ (getTransactionId() >>> 32));
    }
    ...
    private boolean checkSignature() {
        if (!hasValidSignature) {
            hasValidSignature = signature != null && Crypto.ecVerify(signature, zeroSignature(getBytes()), getSenderPublicKey(), useNQT());
        }
    ...
    public int getFullSize() {
        ...
        for (AbstractEnclosure appendage : getAppendages()) {
    ...
    private boolean useNQT() {
        return this.height > Constants.EC_NQT_BLOCK
                && (this.timestamp > 0
                || EcBlockchainImpl.getInstance().getHeight() >= Constants.EC_NQT_BLOCK);
    }
    ...
    public void validate() throws EcValidationException {
        if (timestamp == 0 ? (deadline != 0 || feeNQT != 0) : (deadline < 1 || feeNQT <= 0)
                || feeNQT > Constants.EC_MAX_BALANCE_NQT
                || amountNQT < 0
                || amountNQT > Constants.EC_MAX_BALANCE_NQT
                || type == null) {
            throw new EcNotValidExceptionEc("Invalid transaction parameters:\n type: " + type + ", timestamp: " + timestamp
        ...
        if (referencedTransactionFullHash != null && referencedTransactionFullHash.length != 32) {
            throw new EcNotValidExceptionEc("Invalid referenced transaction full hash " + Convert.toHexString(referencedTransactionFullHash));
        }

        if (attachment == null || type != attachment.getTransactionType()) {
            throw new EcNotValidExceptionEc("Invalid attachment " + attachment + " for transaction of type " + type);
        }

        if (!type.canHaveRecipient()) {
            if (recipientId != 0 || getAmountNQT() != 0) {
                throw new EcNotValidExceptionEc("Transactions of this type must have recipient == 0, amount == 0");
            }
        }

        if (type.mustHaveRecipient() && version > 0) {
            if (recipientId == 0) {
                throw new EcNotValidExceptionEc("Transactions of this type must have a valid recipient");
            }
        }

        boolean validatingAtFinish = phasing != null && getSignature() != null && PhasingPoll.getPoll(getTransactionId()) != null;
        for (AbstractEnclosure appendage : appendages) {
            appendage.loadPrunable(this);
            if (!appendage.verifyVersion(this.version)) {
                throw new EcNotValidExceptionEc("Invalid attachment version " + appendage.getEcVersion()
        ...
        if (getFullSize() > Constants.EC_MAX_PAYLOAD_LENGTH) {
            throw new EcNotValidExceptionEc("Transaction size " + getFullSize() + " exceeds maximum payload size");
        }
        ...
        if (!validatingAtFinish) {
            ...
            if (feeNQT < minimumFeeNQT) {
                throw new EcNotCurrentlyValidExceptionEc(String.format("Transaction fee %f EC less than minimum fee %f EC at height %d",
                        ((double) feeNQT) / Constants.ONE_EC, ((double) minimumFeeNQT) / Constants.ONE_EC, blockchainHeight));
            }
            if (blockchainHeight > Constants.EC_FXT_BLOCK && ecBlockId != 0) {
                if (blockchainHeight < ecBlockHeight) {
                    throw new EcNotCurrentlyValidExceptionEc("ecBlockHeight " + ecBlockHeight
                            + " exceeds EC_BLOCKCHAIN height " + blockchainHeight);
                }
                if (EcBlockH2.findBlockIdAtHeight(ecBlockHeight) != ecBlockId) {
                    throw new EcNotCurrentlyValidExceptionEc("ecBlockHeight " + ecBlockHeight
    ...
    void apply() {
        ...
        if (referencedTransactionFullHash != null
                && timestamp > Constants.EC_REFERENCED_TRANSACTION_FULL_HASH_BLOCK_TIMESTAMP) {
            senderAccount.addToUnconfirmedBalanceNQT(getTransactionType().getLedgerEvent(), getTransactionId(),
                    0, Constants.EC_UNCONFIRMED_POOL_DEPOSIT_NQT);
        }
        if (attachmentIsPhased()) {
            senderAccount.addToBalanceNQT(getTransactionType().getLedgerEvent(), getTransactionId(), 0, -feeNQT);
        }
        for (AbstractEnclosure appendage : appendages) {
    ...
    private long getMinimumFeeNQT(int blockchainHeight) {
        ...
        if (referencedTransactionFullHash != null) {
            totalFee = Math.addExact(totalFee, Constants.ONE_EC);
        }
        return totalFee;
    }
    ...
    public static final class BuilderImpl implements Builder {
        ...
        private final TransactionType type;
        private final byte version;
        private Mortgaged.AbstractMortgaged attachment;

        private long recipientId;
        private byte[] referencedTransactionFullHash;
        private byte[] signature;
        private Message message;
        private Enclosure.EncryptedMessage encryptedMessage;
        private Enclosure.EncryptToSelfMessage encryptToSelfMessage;
        ...
        private Enclosure.PrunableEncryptedMessage prunableEncryptedMessage;
        ...
        public BuilderImpl(byte version, byte[] senderPublicKey, long amountNQT, long feeNQT, short deadline,
                           Mortgaged.AbstractMortgaged attachment) {
        ...
        @Override
        public TransactionImpl build(String secretPhrase) throws EcNotValidExceptionEc {
            if (timestamp == Integer.MAX_VALUE) {
                timestamp = new EcTime.EpochEcTime().getTime();
            }
            if (!ecBlockSet) {
                EcBlock ecEcBlock = EcBlockchainImpl.getInstance().getECBlock(timestamp);
                this.ecBlockHeight = ecEcBlock.getHeight();
                this.ecBlockId = ecEcBlock.getECId();
            }
            return new TransactionImpl(this, secretPhrase);
        }

        @Override
        public TransactionImpl build() throws EcNotValidExceptionEc {
            return build(null);
        }
        ...
        public BuilderImpl appendix(Mortgaged.AbstractMortgaged attachment) {
            this.attachment = attachment;
            return this;
        }
        ...
        @Override
        public BuilderImpl appendix(Enclosure.EncryptedMessage encryptedMessage) {
            this.encryptedMessage = encryptedMessage;
            return this;
        }

        @Override
        public BuilderImpl appendix(Enclosure.EncryptToSelfMessage encryptToSelfMessage) {
        ...
        @Override
        public BuilderImpl appendix(Enclosure.PrunableEncryptedMessage prunableEncryptedMessage) {
