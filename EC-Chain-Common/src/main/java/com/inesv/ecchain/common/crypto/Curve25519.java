...
final class Curve25519 {
    ...
    private static final byte[] ORDER_TIMES = {
    ...
    public static final byte[] ECORDER = {
    ...
    public static boolean sign(byte[] v, byte[] h, byte[] x, byte[] s) {
        ...
        ec_divmod(tmp3, h1, 32, ECORDER, 32);
        ec_divmod(tmp3, x1, 32, ECORDER, 32);

        // v = x1 - h1
        // If v is negative, add the group order to it to become positive.
        // If v was already positive we don't have to worry about overflow
        // when adding the order because v < ECORDER and 2*ECORDER < 2^256
        ec_mula_small(v, x1, 0, h1, 32, -1);
        ec_mula_small(v, v, 0, ECORDER, 32, 1);

        // tmp1 = (x-h)*s mod q
        ec_mula32(tmp1, v, s, 32, 1);
        ec_divmod(tmp2, tmp1, 64, ECORDER, 32);
    ...
    public static void ecverify(byte[] Y, byte[] v, byte[] h, byte[] P) {
        /* Y = v abs(P) + h G  */
        byte[] d = new byte[32];
        long10[]
        ...
        ec_mul(t2[1], BASE_2Y, t1[0]);/* t2[1] = 2 Py Gy or -2 Py Gy  */
        ...
        ec_mul(s[0], t1[0], t2[0]);	/* s[0] = t1[0]/(Px - Gx)^2  */
        sub(s[0], s[0], p[1]);	/* s[0] = t1[0]/(Px - Gx)^2 - Px  */
        s[0]._0 -= 9 + 486662;		/* s[0] = X(P+G)  */
        ec_mul(s[1], t1[1], t2[0]);	/* s[1] = t1[1]/(Px - Gx)^2  */
        sub(s[1], s[1], p[1]);	/* s[1] = t1[1]/(Px - Gx)^2 - Px  */
        s[1]._0 -= 9 + 486662;		/* s[1] = X(P-G)  */
        ec_mul_small(s[0], s[0], 1);	/* reduce s[0] */
        ec_mul_small(s[1], s[1], 1);	/* reduce s[1] */
    ...
    public static boolean isCanonicalSignature(byte[] v) {
        ...
        ec_divmod(tmp, vCopy, 32, ECORDER, 32);
        for (int i = 0; i < 32; i++) {
            if (v[i] != vCopy[i]) {
                return false;
            }
    ...
    private static int ec_mula_small(byte[] p, byte[] q, int m, byte[] x, int n, int z) {
        int v = 0;
        for (int i = 0; i < n; ++i) {
    ...
    private static int ec_mula32(byte[] p, byte[] x, byte[] y, int t, int z) {
        final int n = 31;
        int w = 0;
        int i = 0;
        for (; i < t; i++) {
            int zy = z * (y[i] & 0xFF);
            w += ec_mula_small(p, p, i, x, n, zy) +
    ...
    private static void ec_divmod(byte[] q, byte[] r, int n, byte[] d, int t) {
        int rn = 0;
        int dt = ((d[t - 1] & 0xFF) << 8);
        if (t > 1) {
            dt |= (d[t - 2] & 0xFF);
        ...
        while (n-- >= t) {
            ...
            z /= dt;
            rn += ec_mula_small(r, r, n - t + 1, d, t, -z);
            q[n - t + 1] = (byte) ((z + rn) & 0xFF); /* rn is 0 or -1 (underflow) */
            ec_mula_small(r, r, n - t + 1, d, t, -rn);
            rn = (r[n] & 0xFF);
            r[n] = 0;
        }
        r[t - 1] = (byte) rn;
    ...
    private static byte[] egcd32(byte[] x, byte[] y, byte[] a, byte[] b) {
        ...
        while (true) {
            qn = bn - an + 1;
            ec_divmod(temp, b, bn, a, an);
            bn = numsize(b, bn);
            if (bn == 0) {
                return x;
            }
            ec_mula32(y, x, temp, qn, -1);

            qn = an - bn + 1;
            ec_divmod(temp, a, an, b, bn);
            an = numsize(a, an);
            if (an == 0) {
                return y;
            }
            ec_mula32(x, y, temp, qn, -1);
        }
    }
    ...
    private static long10 ec_mul_small(long10 xy, long10 x, long y) {
        long t;
        t = (x._8 * y);
        xy._8 = (t & ((1 << 26) - 1));
        t = (t >> 26) + (x._9 * y);
    ...
    private static long10 ec_mul(long10 xy, long10 x, long10 y) {
        /* sahn0:
    ...
    private static void recip(long10 y, long10 x, int sqrtassist) {
        ...
        sqr(t2, t1);	/*  4 == 2 * 2	*/
        sqr(t0, t2);	/*  8 == 2 * 4	*/
        ec_mul(t2, t0, x);	/*  9 == 8 + 1	*/
        ec_mul(t0, t2, t1);	/* 11 == 9 + 2	*/
        sqr(t1, t0);	/* 22 == 2 * 11	*/
        ec_mul(t3, t1, t2);	/* 31 == 22 + 9
                    == 2^5   - 2^0	*/
        sqr(t1, t3);	/* 2^6   - 2^1	*/
        sqr(t2, t1);	/* 2^7   - 2^2	*/
        sqr(t1, t2);	/* 2^8   - 2^3	*/
        ...
        ec_mul(t3, t4, t1);	/* 2^40  - 2^0	*/
        for (i = 0; i < 5; i++) {
            sqr(t1, t3);
            sqr(t3, t1);
        } /* t3 */		/* 2^50  - 2^10	*/
        ec_mul(t1, t3, t2);	/* 2^50  - 2^0	*/
        sqr(t2, t1);	/* 2^51  - 2^1	*/
        sqr(t3, t2);	/* 2^52  - 2^2	*/
        for (i = 1; i < 25; i++) {
            sqr(t2, t3);
        ...
        ec_mul(t3, t4, t2);	/* 2^200 - 2^0	*/
        for (i = 0; i < 25; i++) {
            sqr(t4, t3);
            sqr(t3, t4);
        } /* t3 */		/* 2^250 - 2^50	*/
        ec_mul(t2, t3, t1);	/* 2^250 - 2^0	*/
        sqr(t1, t2);	/* 2^251 - 2^1	*/
        sqr(t2, t1);	/* 2^252 - 2^2	*/
        if (sqrtassist != 0) {
            ec_mul(y, x, t2);	/* 2^252 - 3 */
        } else {
            sqr(t1, t2);	/* 2^253 - 2^3	*/
            sqr(t2, t1);	/* 2^254 - 2^4	*/
            sqr(t1, t2);	/* 2^255 - 2^5	*/
            ec_mul(y, t1, t0);	/* 2^255 - 21	*/
        }
    }
    ...
    private static void sqrt(long10 x, long10 u) {
        ...
        sqr(x, v);		/* x = v^2		*/
        ec_mul(t2, t1, x);	/* t2 = 2uv^2		*/
        t2._0--;		/* t2 = 2uv^2-1		*/
        ec_mul(t1, v, t2);	/* t1 = v(2uv^2-1)	*/
        ec_mul(x, u, t1);	/* x = uv(2uv^2-1)	*/
    }
    ...
    private static void core(byte[] Px, byte[] s, byte[] k, byte[] Gx) {
        ...
        if (s != null) {
            ...
            ec_mul(t2, x[1], t3);	/* t2 = Qx  */
            add(t2, t2, dx);	/* t2 = Qx + Px  */
            t2._0 += 9 + 486662;	/* t2 = Qx + Px + Gx + 486662  */
            dx._0 -= 9;		/* dx = Px - Gx  */
            sqr(t3, dx);	/* t3 = (Px - Gx)^2  */
            ec_mul(dx, t2, t3);	/* dx = t2 (Px - Gx)^2  */
            sub(dx, dx, t1);	/* dx = t2 (Px - Gx)^2 - Py^2  */
            dx._0 -= 39420360;	/* dx = t2 (Px - Gx)^2 - Py^2 - Gy^2  */
            ec_mul(t1, dx, BASE_R2Y);	/* t1 = -Py  */
            if (is_negative(t1) != 0)	/* ecSign is 1, so just copy  */ {
                cpy32(s, k);
            } else			/* ecSign is -1, so negate  */ {
                ec_mula_small(s, ORDER_TIMES, 0, k, 32, -1);
            }

            /* reduce s mod q
             * (is this needed?  do it just in case, it's fast anyway) */
            //ec_divmod((dstptr) t1, s, 32, order25519, 32);
            ...
            if ((s[31] & 0x80) != 0) {
                ec_mula_small(s, s, 0, ECORDER, 32, 1);
            }
        }
    }
