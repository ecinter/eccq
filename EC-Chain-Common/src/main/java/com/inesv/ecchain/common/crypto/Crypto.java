...
public final class Crypto {
    ...
    private static final boolean USE_STRONG_SECURE_RANDOM = PropertiesUtil.getKeyForBoolean("ec.useStrongSecureRandom");

    private static final ThreadLocal<SecureRandom> SECURE_RANDOM = new ThreadLocal<SecureRandom>() {
        @Override
        protected SecureRandom initialValue() {
            try {
                SecureRandom secureRandom = USE_STRONG_SECURE_RANDOM ? SecureRandom.getInstanceStrong() : new SecureRandom();
                secureRandom.nextBoolean();
                return secureRandom;
            } catch (NoSuchAlgorithmException e) {
                LoggerUtil.logError("No secure random provider available");
                throw new RuntimeException(e.getMessage(), e);
            }
        }
    };

    public static SecureRandom getEcSecureRandom() {
        return SECURE_RANDOM.get();
    }

    public static MessageDigest getEcMessageDigest(String algorithm) {
        try {
            return MessageDigest.getInstance(algorithm);
        } catch (NoSuchAlgorithmException e) {
            LoggerUtil.logInfo("Missing message digest algorithm: " + algorithm);
            throw new RuntimeException(e.getMessage(), e);
        }
    }

    public static MessageDigest sha256() {
        return getEcMessageDigest("SHA-256");
    }
    ...
    public static byte[] getEcKeySeed(String secretPhrase, byte[]... nonces) {
    ...
    public static byte[] ecSign(byte[] message, String secretPhrase) {
        byte[] P = new byte[32];
        byte[] s = new byte[32];
        MessageDigest digest = Crypto.sha256();
    ...
    public static boolean ecVerify(byte[] signature, byte[] message, byte[] publicKey, boolean enforceCanonical) {
        try {
            if (signature.length != 64) {
                return false;
            }
            if (enforceCanonical && !Curve25519.isCanonicalSignature(signature)) {
                LoggerUtil.logDebug("Rejecting non-canonical signature");
                return false;
            }

            if (enforceCanonical && !Curve25519.isCanonicalPublicKey(publicKey)) {
                LoggerUtil.logDebug("Rejecting non-canonical public key");
                return false;
            }
            ...
            Curve25519.ecverify(Y, v, h, publicKey);
            ...
            LoggerUtil.logError("Error verifying signature", e);
            return false;
        }
    }
    ...
    private static byte[] getSharedSecret(byte[] myPrivateKey, byte[] theirPublicKey) {
        try {
            ...
            LoggerUtil.logError("Error getting shared secret", e);
            throw e;
        }
    }
    ...
    public static String ecRsEncode(long id) {
        return ReedSolomon.encode(id);
    }

    public static long ecRsDecode(String rsString) {
        ...
        try {
            ...
            LoggerUtil.logDebug("Reed-Solomon decoding failed for " + rsString + ": " + e.toString());
