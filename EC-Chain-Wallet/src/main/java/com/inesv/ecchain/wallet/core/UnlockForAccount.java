...
public final class UnlockForAccount extends UserRequestHandler {

    static final UnlockForAccount instance = new UnlockForAccount();

    private UnlockForAccount() {
    }
    ...
    JSONStreamAware processRequest(HttpServletRequest req, User user) throws IOException {
        ...
        Users.getAllEcUsers().forEach(u -> {
            if (secretPhrase.equals(u.getSecretECPhrase())) {
                u.lockAccount();
                if (!u.isInactive()) {
                    u.enqueue(EC_LOCK_ACCOUNT);
                }
            }
        });
        ...
        if (account == null) {
            ...
            int blockchainHeight = bc.getLastECBlock().getHeight();
            try (H2Iterator<? extends EcBlock> blockIterator = EcBlockchainImpl.getInstance().getBlocks(accountId, 0)) {
                while (blockIterator.hasNext()) {
                    EcBlock ecBlock = blockIterator.next();
                    if (ecBlock.getTotalFeeNQT() > 0) {
                        JSONObject myTransaction = new JSONObject();
                        myTransaction.put("index", "ecBlock" + Users.getIndex(ecBlock));
                        myTransaction.put("blockTimestamp", ecBlock.getTimestamp());
                        myTransaction.put("ecBlock", ecBlock.getStringECId());
                        myTransaction.put("earnedAmountNQT", ecBlock.getTotalFeeNQT());
                        myTransaction.put("numberOfConfirmations", blockchainHeight - ecBlock.getHeight());
                        myTransaction.put("id", "-");
                        myTransaction.put("timestamp", ecBlock.getTimestamp());
                        myTransactionsSet.add(myTransaction);
                    }
                }
            }

            try (H2Iterator<? extends Transaction> transactionIterator = EcBlockchainImpl.getInstance().getTransactions(accountId, (byte) -1, (byte) -1, 0, false)) {
                while (transactionIterator.hasNext()) {
                    ...
                    if (transaction.getSenderId() == accountId) {
                        ...
                        myTransaction.put("feeNQT", transaction.getFeeNQT());
                        myTransaction.put("numberOfConfirmations", blockchainHeight - transaction.getTransactionHeight());
                        myTransaction.put("id", transaction.getStringId());
